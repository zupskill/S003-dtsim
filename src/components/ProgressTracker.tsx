...
export default function ProgressTracker({ currentStage, setStage, maxReachedStage }: ProgressTrackerProps) {
  ...
  return (
    <div className="w-full bg-surface/80 backdrop-blur-md border-b border-border py-3 px-4 sticky top-0 z-40 shadow-lg shadow-brand-primary/10">
            ...
            <React.Fragment key={stage.num}>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all duration-300 relative shrink-0 ${
                  isCurrent
                    ? "bg-gradient-to-r from-brand-primary to-brand-primary/80 text-text-primary shadow-[0_0_15px_rgba(0,181,230,0.5)] font-medium scale-105"
                    : isCompleted
                    ? "bg-surface-hover/80 text-brand-secondary font-medium cursor-pointer hover:bg-surface-hover border border-brand-secondary/20 shadow-[0_0_8px_rgba(255,200,61,0.1)]"
                    : isPlayable
              ...
              >
                <span
                  className={`w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0 ${
                    isCurrent
                      ? "bg-background text-brand-primary font-bold"
                      : isCompleted
                ...
                <div
                  className={`h-0.5 min-w-[12px] flex-1 mx-2 transition-all duration-500 rounded-full ${
                    stage.num < currentStage
                      ? "bg-gradient-to-r from-brand-primary/50 to-brand-primary/30 shadow-[0_0_6px_rgba(0,181,230,0.3)]"
                      : "bg-surface-hover"
                  }`}
                />
              )}
